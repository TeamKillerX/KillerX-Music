...
from KillerXMusic.utils.database import get_loop
from KillerXMusic.utils.decorators import AdminRightsCheck
from KillerXMusic.utils.inline.play import (stream_markup,
                                          telegram_markup)
...
async def skip(cli, message: Message, _, chat_id):
    ...
    if "live_" in queued:
        ...
        try:
            await KillerX.skip_stream(chat_id, file_path, video=status)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        try:
            await KillerX.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
        button = telegram_markup(_, chat_id)
        run = await message.reply_photo(
